...
2. Tests d'intégration API
   ...
   - Coverage cible : 80%
3. Tester l'endpoint dans Postman
   - Importer la collection si nécessaire (`docs/postman/`)
   - Vérifier que la requête fonctionne correctement
   - Vérifier que les tests automatisés passent

#### Étape 4 : Postman Collection

1. Ajouter/Modifier la requête dans la collection Postman
   - Ouvrir `docs/postman/Vectra_API_Collection.json` dans Postman
   - Créer/modifier la requête dans le bon dossier (Auth, Campaigns, etc.)
   - Ajouter tests automatisés (code statut, structure réponse)
   - Sauvegarder automatiquement les tokens si nécessaire
2. Exporter la collection (Menu → Export)
3. Commit les changements : `docs(postman): add/update endpoint [nom]`
